...
- Using a lookup cell containing a real Invoice ID (`351-62-0822`), build `=VLOOKUP(F1,RawSales[#All],2,FALSE)` to pull back its Branch (Cairo); leave FALSE out once first to show the wrong approximate-match result, then add it back
...
> "All three just returned Cairo. The difference isn't the answer, it's which one still works in five years, on an older machine, or when someone inserts a column."
...
- Build the plain aggregates first: `=SUM(RawSales[Sales])`, `=AVERAGE(RawSales[Rating])`, `=COUNT(RawSales[Sales])`
- Introduce the conditional versions as "the same idea, but only counting rows that match a rule": `=COUNTIF(RawSales[Rating],">=7")` should return 501, and `=SUMIF(RawSales[Branch],"Alex",RawSales[Sales])` should return 106,200.37
...
- Step 3: `=COUNTIF()` on the satisfaction column, checked against the total row count of 1,000 (aggregate function)

**Ask students:** which function family would you reach for if the question was "how many Health and beauty transactions happened in the Cairo branch?"
...
| Using SUM or COUNT when the question actually needed a conditional version | "Reread the question. If it names a condition, Branch equals Alex, Rating above 7, you need the IF-suffixed version." |
