...
1. Open the cleaned 1,000-row file from Topic 2.3, with its verified row count intact
...
3. Have a finished, pre-built copy of the workbook with the finished cross-tab, slicer and verified totals already in place, as a fallback
...
- Confirm it defaulted to Sum, not Count, and sort the result largest to smallest, showing Giza leading at 110,568.71
...
- Drag Branch into the Filters area, set the dropdown to Giza only, and narrate explicitly: "the source data has not changed, only what this pivot table currently shows has"
...
- PivotTable Analyze tab, Insert Slicer, choose Branch, and click through Alex, Cairo and Giza, holding Ctrl to select two at once
...
- Pick the Giza total from Part 1 and recreate it independently: `=SUMIF(RawSales[Branch],"Giza",RawSales[Sales])`
- Confirm both figures read 110,568.71
...
A PivotTable sheet containing: a Branch-by-Sales summary (Sum, sorted, Giza leading at 110,568.71), a Branch-by-Product-line cross-tab, a Rating summary corrected to Average, a working Branch slicer, and a documented verification showing the Giza total matching a manual SUMIF. This pivot table is what Topic 2.5's charts and dashboard are built directly from.
