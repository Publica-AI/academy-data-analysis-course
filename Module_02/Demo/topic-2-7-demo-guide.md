...
1. Carry forward the cleaned 1,000-row dataset and the verified formulas built across Topics 2.2-2.6
...
- State the scenario aloud: "Total Sales for the Alex branch only, from the cleaned dataset"
- Prompt the AI, naming the Branch and Sales columns and the condition explicitly, and note the figure it returns
- Build the manual check independently: `=SUMIF(Branch_range,"Alex",Sales_range)`, which should return 106,200.37
...
- Compare its suggested order against the Trim-before-Remove-Duplicates lesson from Topic 2.6, and check the row count comes out to the expected 1,000 before trusting the suggestion
- Instructor only: open Copilot in Excel, select the Sales column, and ask it the same Alex branch total question from Part 4, showing the same figure arriving without any copy-pasting between windows
...
The cleaned dataset from Topics 2.3-2.6, with an AI-generated and verified Satisfaction formula, an AI-explained nested INDEX-MATCH-IF formula with a tested prediction, a debugged VLOOKUP with its fix applied and confirmed, and a documented Alex branch total (106,200.37) matching between an AI-generated formula and a manual SUMIF. This is the final state of the module's running workbook.
...
> **Instructor note:** the confirmed "Up Next" text for the end of Module 2 (the module wrap-up, assessment, or Module 3) has not yet been finalised by the curriculum team, per the note carried over from the topic's own slides. Please update this line once that is confirmed rather than leaving a placeholder or invented title in place.
