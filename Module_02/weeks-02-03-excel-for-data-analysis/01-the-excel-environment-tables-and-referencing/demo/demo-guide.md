...
1. Open the raw supermarket sales export (`Ilesanmi_Sales_Raw_Export.csv`, 1,025 rows) in a fresh workbook, as a plain range, not yet a table
...
`RawSales` table (1,025 rows) with two new columns: Subtotal (relative reference, `=[@[Unit price]]*[@Quantity]`) and a verified Tax Check column (`=Subtotal*$B$1`, matching the dataset's real Tax 5% column row by row on 1,020 of the 1,025 rows). This table carries forward into Topic 2.2 as the file trainees build their formulas on.

> **Instructor note:** the two columns match on every row a trainee is likely to sample, and their totals do not: `=SUM(RawSales[Tax Check])` reads ₦1,565,351.20 against ₦1,578,894.40 for Tax 5%. The ₦13,543.20 gap is five per cent of the ₦270,864.00 gap between Subtotal and cogs, and both come from the same five sign-flipped quantities repaired in Topic 2.3. If a trainee spots it, that is a good outcome, not a mistake to talk them out of.
